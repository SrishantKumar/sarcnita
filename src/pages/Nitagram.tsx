...
import { Search, TrendingUp, Clock, Filter, LogIn } from 'lucide-react';
...
const Nitagram: React.FC = () => {
  ...
  const filteredPosts = posts
    .filter(post => {
      ...
      return post.space === activeSpace;
    })
    .filter(post => {
      if (!searchQuery) return true;
      return (
        post.content.toLowerCase().includes(searchQuery.toLowerCase()) ||
        post.space?.toLowerCase().includes(searchQuery.toLowerCase())
      );
    })
    .sort((a, b) => {
      if (sortBy === 'recent') {
      ...
      <div className="pt-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <div 
              className={`
                fixed md:relative inset-0 bg-black bg-opacity-50 z-20 md:bg-transparent md:z-auto
                ${isSidebarOpen ? 'block' : 'hidden md:block'}
              `}
              onClick={() => setIsSidebarOpen(false)}
            >
              <div 
                className="w-64 h-full bg-white md:rounded-xl md:shadow-md overflow-y-auto"
                  ...
                  {filteredPosts.map((post) => (
                    ...
                    >
                      <PostCard
                        ...
                        onDelete={user && (user.id === post.user_id || profile?.role === 'admin') ? deletePost : undefined}
                      />
                    </motion.div>
                  ))}
                </motion.div>
